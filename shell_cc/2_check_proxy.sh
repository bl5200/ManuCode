...
declare availableproxyfile="proxylist_available.txt2"
declare line
declare times
declare ip
declare port
...
if [ -r "$availableproxyfile" ]
then
  rm -f $availableproxyfile
fi
touch $availableproxyfile
...
do
    ...
    # start test
    if GET -t 5 -p "http://$ip:$port" "http://baidu.com" &>/dev/null	#emphasis
    then 
        echo "$ip $port" >> $availableproxyfile
...
exec 0>&-
echo "=====report : `cat $availableproxyfile | wc -l` proxy_soldiers is available====="
quit 0
#exit
