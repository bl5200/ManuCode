...
then
    ...
    echo "usage: bash $0 proxyListFile.txt attack_url"
    echo "error: must have two input arg"
    quit 1
fi
...
do
    ...
    do
        ...
        for i in `echo $line | awk '{print $1,$2}'`
        do
            j=$((j+1))
            if [ "$j" -eq 1 ]
            then
        ...
        if GET -t "$get_timeout_sec" -p "http://$ip:$port" "$target_url" &>/dev/null	#emphasis
        then 
